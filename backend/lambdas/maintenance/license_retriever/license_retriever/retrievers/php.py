...
async def retrieve_php_licenses_batch(packages: list[tuple[str, str]], max_concurrent: int = 10) -> dict[str, list[str]]:
    """
...
async def process_php_licenses(session: aiohttp.ClientSession, package_info: dict, name: str) -> list[str]:
    ...
    if not licenses and version_data.get("homepage", "").startswith("https://github.com"):
        ...
        repo_license = get_license(version_data["homepage"])
        if repo_license:
