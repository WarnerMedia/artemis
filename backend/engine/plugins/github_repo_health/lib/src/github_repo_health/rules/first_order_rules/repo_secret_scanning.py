...
class RepoSecretScanning:
    ...
    def check(github, owner, repo, branch=None, config={}):
        try:
            ...
            return add_metadata(
                False, RepoSecretScanning, config, error_message=e.data.get("message")
            )
        ...
        secret_scanning_enabled = (
            security_and_analysis.get("secret_scanning", {}).get("status") == "enabled"
        )

        push_protection_not_configured = (
            config.get("require_push_protection") is not True
        )
        push_protection_enabled = (
            security_and_analysis.get("secret_scanning_push_protection", {}).get(
                "status"
            )
            == "enabled"
        )

        passing = secret_scanning_enabled and (
            push_protection_not_configured or push_protection_enabled
        )
